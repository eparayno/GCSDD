...
// Date: April 16, 2020
// Description: Convex Hull program that uses Graham's algorithm. Uses pipes, forks, and recursion to parallelize a process.
...
bool leftturn(Point &a, Point &b, Point &c) {
    ...
    } else if ((slope_ac = (c.y - a.y) / (c.x - a.x)) > 0) {                        // slope_ac > 0
...
void divide_and_conquer(deque <Point> &q, int leaves = 0) {
    ...
    if (leaves > 1) {
        ...
        // print error if pipe not created
        if (pipe(pipeFD) < 0) {
        ...
        if (pid == 0) {
            ...
            if (s2.size() > 1) {
                divide_and_conquer(s2, leaves / 2); // work on the right side
            }

            close(pipeFD[0]);            // close read side of pipe
            send(pipeFD[1], s2);         // behaves as "write", writes s2 for parent to receive later
        ...
        // This will be the parent process
            ...
            if (s1.size() > 1) {
                divide_and_conquer(s1, (leaves - (leaves / 2)));  // work on the left queue
            }

            close(pipeFD[1]);            // close write side of pipe
            s2.clear();                  // clear prior to receiving
            recv(pipeFD[0], s2);         // behaves as "read", reads s2 from child
        }

    }
    // handles case when there is no process argument
    else {
        if (s1.size() > 1) {
            divide_and_conquer(s1, leaves);  // work on the left queue
        }
        if (s2.size() > 1) {
            divide_and_conquer(s2, leaves);  // work on the right queue
        }
    }
